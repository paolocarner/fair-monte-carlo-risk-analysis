...
def compute_sensitivity(
    tef_min: int, tef_mode: int, tef_max: int, total_vulnerability: float,
    ...
    def _run(tef_mode_v, vuln_v, pri_mode_v, sec_mode_v, sec_prob_v):
        ...
        tef_mode_clamped = float(np.clip(tef_mode_v, tef_min, tef_max))
        try:
            tef = FAIRDistribution("pert", tef_min, tef_mode_clamped, tef_max)
    ...
    baseline_args = (tef_mode, total_vulnerability, primary_mode, secondary_mode, secondary_prob)
...
with st.sidebar:
    ...
    if scenario_preset != "Custom" and scenario_preset != st.session_state.last_preset:
        ...
        st.session_state.cfg_tef_min          = p["tef_min"]
        st.session_state.cfg_tef_mode         = p["tef_mode"]
        st.session_state.cfg_tef_max          = p["tef_max"]
        st.session_state.cfg_vuln_contact_pct = p["vuln_contact"] * 100.0
        st.session_state.cfg_vuln_action_pct  = p["vuln_action"]  * 100.0
        st.session_state.cfg_vuln_rate_pct    = p["vuln_rate"]    * 100.0
    ...
    if uploaded_config is not None:
        if uploaded_config.name != st.session_state.last_loaded_config_name:
            try:
                ...
                if p:
                    st.session_state.cfg_tef_min           = int(p.get("tef_min", 100))
                    st.session_state.cfg_tef_mode          = int(p.get("tef_mode", 300))
                    st.session_state.cfg_tef_max           = int(p.get("tef_max", 1000))
                    st.session_state.cfg_vuln_contact_pct  = float(p.get("vuln_contact_pct", 30.0))
                    st.session_state.cfg_vuln_action_pct   = float(p.get("vuln_action_pct", 10.0))
                    st.session_state.cfg_vuln_rate_pct     = float(p.get("vuln_rate_pct", 30.0))
...
with col1:
    ...
    with st.container(border=True):
        ...
        with st.expander("📊 Preview TEF distribution"):
    ...
    with col4:
        ...
        config_data = {
            ...
            "parameters": {
                ...
                "tef_mode": tef_mode,
                "tef_max": tef_max,
                "vuln_contact_pct": round(vuln_contact * 100, 2),
                "vuln_action_pct": round(vuln_action * 100, 2),
                "vuln_rate_pct": round(vuln_rate * 100, 2),
