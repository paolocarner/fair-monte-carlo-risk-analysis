...
# Track last selected preset to detect changes
if 'last_preset' not in st.session_state:
    st.session_state.last_preset = None
# Track loaded config file to avoid infinite reruns
...
# Input validation
# ---------------------------------------------------------------------------
def validate_inputs(cf_min, cf_mode, cf_max,
                    primary_min, primary_mode, primary_max,
                    secondary_min, secondary_mode, secondary_max) -> list:
    errors = []
    if not (cf_min <= cf_mode <= cf_max):
        errors.append(f"Contact Frequency: min ({cf_min:,}) must be ≤ mode ({cf_mode:,}) ≤ max ({cf_max:,})")
...
def generate_html_report(
    ...
    cf_min, cf_mode, cf_max, poa, tef_min, tef_mode, tef_max, total_vulnerability,
    primary_min, primary_mode, primary_max,
...
<div class="params-grid">
  ...
  <div class="param"><strong>Contact Frequency (min / mode / max)</strong>{cf_min:,} / {cf_mode:,} / {cf_max:,} contacts/yr</div>
  <div class="param"><strong>Probability of Action</strong>{poa*100:.1f}%</div>
  <div class="param"><strong>TEF — derived (min / mode / max)</strong>{tef_min:,.1f} / {tef_mode:,.1f} / {tef_max:,.1f}</div>
  <div class="param"><strong>Vulnerability</strong>{total_vulnerability*100:.4f}%</div>
...
<p style="color:#888; font-size:0.85em;">
  Per the Open Group O-RT Standard v3.0.1 §4.3.1: TEF = Contact Frequency × Probability of Action.
  Vulnerability (§4.3.2) is the probability a Threat Event becomes a Loss Event, applied on top of TEF.
</p>

<footer>
  Generated by BARE Cybersecurity FAIR Analysis Tool &nbsp;|&nbsp; Version 1.3<br>
...
def compute_sensitivity(
    cf_min: float, cf_mode: float, cf_max: float, poa: float, vulnerability: float,
    primary_min: int, primary_mode: int, primary_max: int,
    secondary_min: int, secondary_mode: int, secondary_max: int,
    secondary_prob: float,
    n_mini: int = 3000, seed: int = 99,
) -> tuple:
    """
    Vary each key parameter ±20%, run mini-sims, return sorted sensitivity rows
    and baseline ALE.

    CF Mode and PoA are now separate levers (rather than a single "TEF Mode"
    lever) because TEF = CF x PoA is derived, not entered directly -- see
    derive_tef_from_contact() in fair_monte_carlo.py and CHANGELOG.md.

    Returns (rows, baseline_ale) where rows is sorted by swing ascending
    (smallest impact first) so reversed() gives tornado order (largest on top).
    """

    def _run(cf_mode_v, poa_v, vuln_v, pri_mode_v, sec_mode_v, sec_prob_v):
        poa_v = float(np.clip(poa_v, 1e-4, 1.0))
        vuln_v = float(np.clip(vuln_v, 1e-4, 1.0))
        sec_prob_v = float(np.clip(sec_prob_v, 0.0, 1.0))

        # Scale min/max proportionally with mode changes
        pri_scale = pri_mode_v / max(primary_mode, 1)
        sec_scale = sec_mode_v / max(secondary_mode, 1)
        pri_min_v = max(1.0, primary_min * pri_scale)
        pri_max_v = max(pri_mode_v * 1.01, primary_max * pri_scale)
        sec_min_v = max(1.0, secondary_min * sec_scale)
        sec_max_v = max(sec_mode_v * 1.01, secondary_max * sec_scale)

        cf_mode_clamped = float(np.clip(cf_mode_v, cf_min, cf_max))
        try:
            tef_min_v, tef_mode_v, tef_max_v = derive_tef_from_contact(
                cf_min, cf_mode_clamped, cf_max, poa_v
            )
            tef = FAIRDistribution("pert", tef_min_v, tef_mode_v, tef_max_v)
            primary = FAIRDistribution("lognormal", pri_min_v, pri_mode_v, pri_max_v)
            secondary = FAIRDistribution("lognormal", sec_min_v, sec_mode_v, sec_max_v)
            s = FAIRMonteCarloSimulation(n_simulations=n_mini, random_seed=seed)
            st_res = s.run_simulation(tef, vuln_v, primary, secondary, sec_prob_v)
            return st_res["ale_mean"]
        except Exception:
            return 0.0

    baseline_args = (cf_mode, poa, vulnerability, primary_mode, secondary_mode, secondary_prob)
    baseline_ale = _run(*baseline_args)

    param_labels = [
        "Contact Frequency Mode",
    ...
    for i, label in enumerate(param_labels):
        args_low = list(baseline_args)
        args_high = list(baseline_args)
        args_low[i] = baseline_args[i] * 0.8
        args_high[i] = baseline_args[i] * 1.2
        ale_low = _run(*args_low)
        ale_high = _run(*args_high)
        rows.append({
            'param': label,
            'ale_low': ale_low,
            'ale_high': ale_high,
            'swing': abs(ale_high - ale_low),
        })

    return sorted(rows, key=lambda x: x['swing']), baseline_ale


# ---------------------------------------------------------------------------
# Distribution preview helper
# ---------------------------------------------------------------------------
def preview_distribution(
    dist_type: str, min_val: float, mode_val: float, max_val: float,
    currency: str = "€", n: int = 2000,
) -> go.Figure:
    """Return a compact Plotly histogram of samples from the given distribution."""
    try:
        dist = FAIRDistribution(dist_type=dist_type, min_val=min_val,
                                mode_val=mode_val, max_val=max_val)
    ...
    fig.update_layout(
        ...
        showlegend=False,
        xaxis=dict(tickformat=",.0f", title=currency if "loss" in dist_type.lower() else ""),
        yaxis=dict(title=""),
...
# ---------------------------------------------------------------------------
# Multi-year risk projection (bootstrap)
# ---------------------------------------------------------------------------
@st.cache_data(show_spinner=False)
def multi_year_projection(
    annual_losses_tuple: tuple,
    years: tuple = (1, 2, 3, 5),
    n_bootstrap: int = 10_000,
    seed: int = 42,
) -> dict:
    """
    Bootstrap multi-year cumulative loss projections.

    annual_losses_tuple: tuple of floats (hashable for st.cache_data)
    Returns dict: {year: {mean, p50, p90, p95, p99}}
    """
    annual_losses = np.array(annual_losses_tuple)
    rng = np.random.default_rng(seed)
    results = {}
    for yr in years:
        samples = rng.choice(annual_losses, size=(n_bootstrap, yr), replace=True)
...
with st.sidebar:
    ...
    # -----------------------------------------------------------------------
    # Preset change detection — update cfg_ keys so widgets pick up new values
    # -----------------------------------------------------------------------
    if scenario_preset != "Custom" and scenario_preset != st.session_state.last_preset:
        p = load_preset(scenario_preset)
        st.session_state.cfg_cf_min             = p["cf_min"]
        st.session_state.cfg_cf_mode            = p["cf_mode"]
        st.session_state.cfg_cf_max             = p["cf_max"]
        st.session_state.cfg_poa_pct            = p["poa"] * 100.0
        st.session_state.cfg_vulnerability_pct  = p["vulnerability"] * 100.0
        st.session_state.cfg_primary_min      = p["primary_min"]
        st.session_state.cfg_primary_mode     = p["primary_mode"]
        st.session_state.cfg_primary_max      = p["primary_max"]
        st.session_state.cfg_secondary_min    = p["secondary_min"]
        st.session_state.cfg_secondary_mode   = p["secondary_mode"]
        st.session_state.cfg_secondary_max    = p["secondary_max"]
        st.session_state.cfg_secondary_prob_pct = p["secondary_prob"] * 100.0
        st.session_state.last_preset = scenario_preset
    ...
    uploaded_config = st.file_uploader(
        "Upload saved config (.json)",
        type=["json"],
        help="Load a previously saved scenario configuration."
    )
    if uploaded_config is not None:
        if uploaded_config.name != st.session_state.last_loaded_config_name:
            try:
                cfg = json.load(uploaded_config)
                p = cfg.get("parameters", {})
                schema_version = cfg.get("schema_version", 1)
                if p:
                    if schema_version >= 2 and "cf_min" in p:
                        # Current schema: CF + PoA entered directly.
                        st.session_state.cfg_cf_min            = int(p.get("cf_min", 1000))
                        st.session_state.cfg_cf_mode           = int(p.get("cf_mode", 3000))
                        st.session_state.cfg_cf_max            = int(p.get("cf_max", 10000))
                        st.session_state.cfg_poa_pct           = float(p.get("poa_pct", 10.0))
                        st.session_state.cfg_vulnerability_pct = float(p.get("vulnerability_pct", 1.0))
                    else:
                        # Legacy (pre-fix) schema: TEF was entered directly and
                        # Vulnerability was Contact% x Action% x Rate% -- a bug
                        # (see CHANGELOG.md). Migrate on a best-effort basis:
                        # keep the old TEF as-is, reuse the old Action% as PoA,
                        # back-derive CF = TEF / PoA, and use only the old
                        # Rate% as Vulnerability (dropping the erroneous extra
                        # Contact% multiplication). This reproduces the same
                        # TEF the config was originally built around, but the
                        # resulting ALE/LEF will differ from the original
                        # (buggy) run, because Vulnerability is no longer
                        # double-discounted.
                        legacy_tef_min = float(p.get("tef_min", 100))
                        legacy_tef_mode = float(p.get("tef_mode", 300))
                        legacy_tef_max = float(p.get("tef_max", 1000))
                        legacy_poa = float(p.get("vuln_action_pct", 10.0)) / 100.0
                        legacy_poa = min(max(legacy_poa, 1e-4), 1.0)
                        legacy_vulnerability_pct = float(p.get("vuln_rate_pct", 30.0))
                        st.session_state.cfg_cf_min  = max(1, round(legacy_tef_min / legacy_poa))
                        st.session_state.cfg_cf_mode = max(1, round(legacy_tef_mode / legacy_poa))
                        st.session_state.cfg_cf_max  = max(1, round(legacy_tef_max / legacy_poa))
                        st.session_state.cfg_poa_pct = legacy_poa * 100.0
                        st.session_state.cfg_vulnerability_pct = legacy_vulnerability_pct
                        st.warning(
                            "⚠️ This config was saved with the pre-fix parameter model "
                            "(Contact Frequency was a %, and Vulnerability double-counted "
                            "it). It has been auto-migrated to the corrected CF × PoA model "
                            "using the old TEF and Action/Rate values, but resulting ALE and "
                            "LEF will differ from the original run — please review before "
                            "using in a client engagement. See CHANGELOG.md for details."
                        )
                    st.session_state.cfg_primary_min       = int(p.get("primary_min", 20000))
                    st.session_state.cfg_primary_mode      = int(p.get("primary_mode", 80000))
                    st.session_state.cfg_primary_max       = int(p.get("primary_max", 300000))
                    st.session_state.cfg_secondary_min     = int(p.get("secondary_min", 10000))
                    st.session_state.cfg_secondary_mode    = int(p.get("secondary_mode", 50000))
                    st.session_state.cfg_secondary_max     = int(p.get("secondary_max", 200000))
                    st.session_state.cfg_secondary_prob_pct = float(p.get("secondary_prob_pct", 40.0))
                    st.session_state.last_loaded_config_name = uploaded_config.name
                    st.session_state.last_preset = None  # prevent preset from overriding
                    st.success(f"✅ Config '{cfg.get('scenario_name', 'unnamed')}' loaded!")
                    st.rerun()
            except Exception as e:
                st.error(f"Failed to load config: {e}")

# ---------------------------------------------------------------------------
# Default values from preset (used only when session_state key doesn't exist)
# ---------------------------------------------------------------------------
...
with col1:
    ...
    with st.container(border=True):
        st.markdown("**🎯 Contact Frequency (CF)**")
        st.caption(
            "Industry-wide threat activity — how often a threat agent contacts your "
            "asset, per year. A COUNT (e.g. malicious emails reaching the org), not a %. "
            "NOT organisation-specific — this is threat-landscape data."
        )
        col_cf1, col_cf2, col_cf3 = st.columns(3)
        with col_cf1:
            cf_min = st.number_input(
                "Minimum contacts/year",
                min_value=1, max_value=1_000_000,
                value=preset_values["cf_min"], step=10,
                key="cfg_cf_min",
                help="Minimum probable number of contacts per year (O-RT §2.4)."
            )
        with col_cf2:
            cf_mode = st.number_input(
                "Most likely contacts/year",
                min_value=1, max_value=1_000_000,
                value=preset_values["cf_mode"], step=10,
                key="cfg_cf_mode",
                help="Most likely (mode) number of contacts per year."
            )
        with col_cf3:
            cf_max = st.number_input(
                "Maximum contacts/year",
                min_value=1, max_value=1_000_000,
                value=preset_values["cf_max"], step=10,
                key="cfg_cf_max",
                help="Maximum probable number of contacts per year."
            )

        with st.expander("📊 Preview Contact Frequency distribution"):
            cf_fig = preview_distribution("pert", cf_min, cf_mode, cf_max)
            if cf_fig:
                st.plotly_chart(cf_fig, use_container_width=True, config={'displayModeBar': False})

    st.markdown("---")
    st.markdown("### 🏢 Internal Factors (Your Organisation)")

    with st.container(border=True):
        st.markdown("**🎯 Threat Event Frequency (TEF) — derived**")
        st.caption("How many of those contacts turn into an actual attempt against you?")
        poa = st.slider(
            "Probability of Action (%)",
            min_value=0.0, max_value=100.0,
            value=preset_values["poa"] * 100, step=1.0,
            key="cfg_poa_pct",
            help="Once contact occurs, probability the threat agent acts (O-RT §2.13). "
                 "Partially controllable — reducing your attack surface/attractiveness lowers this.",
        ) / 100

        # TEF = CF x PoA, per O-RT Standard v3.0.1 §4.3.1 — actually computed now,
        # not just displayed as a caption next to unrelated numbers.
        # cf_min/mode/max may be temporarily out of order while the user is
        # editing (e.g. min > max mid-edit); validate_inputs() below is the
        # source of truth for disabling the Run button, but we must not let
        # an invalid ordering crash the whole script before validation runs.
        try:
            tef_min, tef_mode, tef_max = derive_tef_from_contact(cf_min, cf_mode, cf_max, poa)
            tef_derivation_error = False
        except ValueError:
            tef_min = tef_mode = tef_max = max(cf_min, cf_mode, cf_max, 1) * poa
            tef_derivation_error = True

        if tef_derivation_error:
            st.caption("📈 TEF = CF × PoA → fix Contact Frequency ordering below to compute TEF")
        else:
            st.caption(
                f"📈 TEF = CF × PoA → min={tef_min:,.1f}, mode={tef_mode:,.1f}, max={tef_max:,.1f} attempts/yr"
            )

        with st.expander("📊 Preview derived TEF distribution"):
            tef_fig = preview_distribution("pert", tef_min, tef_mode, tef_max)
            if tef_fig:
                st.plotly_chart(tef_fig, use_container_width=True, config={'displayModeBar': False})

    with st.container(border=True):
        st.markdown("**🔓 Vulnerability (Your Controls)**")
        st.caption(
            "Probability a threat event (an attempt already counted in TEF above) "
            "succeeds and becomes a loss event. Directly controllable via your security posture."
        )
        total_vulnerability = st.slider(
            "Vulnerability (%)",
            min_value=0.0, max_value=100.0,
            value=preset_values["vulnerability"] * 100, step=0.01,
            key="cfg_vulnerability_pct",
            help="P(Threat Capability > Resistance Strength) — O-RT §4.3.2. "
                 "Not Contact Frequency, and not re-multiplied by it."
        ) / 100
...
with col2:
    ...
    with st.container(border=True):
        ...
        with st.expander("📊 Preview primary loss distribution"):
            pri_fig = preview_distribution("lognormal", primary_min, primary_mode, primary_max, currency)
            if pri_fig:
                st.plotly_chart(pri_fig, use_container_width=True, config={'displayModeBar': False})
    ...
    with st.container(border=True):
        ...
        secondary_prob = st.slider(
            ...
            key="cfg_secondary_prob_pct",
            help="Probability a primary event also triggers secondary losses."
        ) / 100

        with st.expander("📊 Preview secondary loss distribution"):
            sec_fig = preview_distribution("lognormal", secondary_min, secondary_mode, secondary_max, currency)
            if sec_fig:
                st.plotly_chart(sec_fig, use_container_width=True, config={'displayModeBar': False})

# ---------------------------------------------------------------------------
# Derived values summary (CF, PoA, and Vulnerability were all set above;
# TEF was already derived via derive_tef_from_contact())
# ---------------------------------------------------------------------------
...
with col_v1:
    st.metric(
        "Derived TEF (mode)",
        f"{tef_mode:,.1f}/yr",
        help="TEF = CF (mode) × Probability of Action, per O-RT §4.3.1."
    )
with col_v2:
    st.metric(
        "Vulnerability",
        f"{total_vulnerability*100:.3f}%",
        help="Probability a threat event becomes a loss event, per O-RT §4.3.2."
    )
with col_v3:
    ...
    st.metric(
        ...
        f"{expected_lef:.2f}",
        help="Loss Event Frequency (LEF) = TEF × Vulnerability, per O-RT §4.3."
    )
...
if st.session_state.simulation_run and st.session_state.stats:
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "📊 Distribution", "📈 Exceedance Curve",
        "🎲 Percentiles", "📉 LEF Analysis", "🌪️ Sensitivity"
    ])
    ...
    # -------------------------------------------------------------------
    # Sensitivity / Tornado chart tab
    # -------------------------------------------------------------------
    with tab5:
        st.markdown(
            "**How much does each parameter drive the ALE?**  "
            "Each bar shows the ALE swing when that parameter is varied ±20% from its current value. "
            "Parameters with wider bars are the highest-leverage levers for risk reduction."
        )
        with st.spinner("Computing sensitivity (±20% per parameter)…"):
            sensitivity_rows, baseline_ale = compute_sensitivity(
                cf_min, cf_mode, cf_max, poa, total_vulnerability,
                primary_min, primary_mode, primary_max,
                secondary_min, secondary_mode, secondary_max, secondary_prob,
            )

        # Tornado: largest swing at top → reverse the sorted list
        sorted_desc = list(reversed(sensitivity_rows))
        y_labels   = [r['param'] for r in sorted_desc]
        ale_lows   = [r['ale_low']  for r in sorted_desc]
        ale_highs  = [r['ale_high'] for r in sorted_desc]

        fig_tornado = go.Figure()

        # Left bars: ALE when parameter is −20% (below baseline)
        fig_tornado.add_trace(go.Bar(
            ...
            marker_color='#2E86AB',
            opacity=0.85,
            text=[f"{currency}{v:,.0f}" for v in ale_lows],
            textposition='inside',
            insidetextanchor='end',
        ))
        # Right bars: ALE when parameter is +20%
        fig_tornado.add_trace(go.Bar(
            y=y_labels,
            x=ale_highs,
            orientation='h',
            ...
            marker_color='#A23B72',
            opacity=0.85,
            text=[f"{currency}{v:,.0f}" for v in ale_highs],
            textposition='inside',
            insidetextanchor='start',
        ))
        # Baseline vertical line
        fig_tornado.add_vline(
            x=baseline_ale, line_dash="dash", line_color="#333333",
            annotation_text=f"Baseline: {currency}{baseline_ale:,.0f}",
            annotation_position="top"
        )
        fig_tornado.update_layout(
            barmode='overlay',
            title="Sensitivity (Tornado) Chart — ALE range per parameter (±20%)",
            xaxis_title=f"Mean Annual Loss Expectancy ({currency})",
            yaxis_title="",
            height=420,
        ...
        sens_df = pd.DataFrame([
            {
                'Parameter': r['param'],
                f'ALE at −20% ({currency})': f"{currency}{r['ale_low']:,.0f}",
                f'Baseline ALE ({currency})': f"{currency}{baseline_ale:,.0f}",
                f'ALE at +20% ({currency})': f"{currency}{r['ale_high']:,.0f}",
                f'Swing ({currency})': f"{currency}{r['swing']:,.0f}",
            }
            for r in sorted_desc
        ])
        st.dataframe(sens_df, hide_index=True, use_container_width=True)
        st.caption(
            "💡 The parameter with the **largest swing** has the most impact on your risk exposure. "
            "Focus security investments on reducing the top driver(s)."
        )
    ...
    # -----------------------------------------------------------------------
    # Multi-year risk projection
    # -----------------------------------------------------------------------
    st.header("📅 Multi-Year Risk Projection")
    st.markdown(
        "Bootstrap projection of **cumulative** losses over multiple years, "
        "by resampling from this simulation's annual loss distribution."
    )

    with st.spinner("Computing multi-year projection…"):
        multi_year = multi_year_projection(
            annual_losses_tuple=tuple(annual_losses.tolist()),
        )

    years = [1, 2, 3, 5]
    percentile_labels = ['mean', 'p50', 'p90', 'p95', 'p99']
    pct_display = {
        'mean': 'Mean',
        'p50':  '50th pct',
        'p90':  '90th pct',
    ...
    fig_multi = go.Figure()
    for pct_key in percentile_labels:
        fig_multi.add_trace(go.Bar(
            x=[f"{yr}yr" for yr in years],
            y=[multi_year[yr][pct_key] for yr in years],
            name=pct_display[pct_key],
            marker_color=pct_colors[pct_key],
            opacity=0.85,
            text=[f"{currency}{multi_year[yr][pct_key]:,.0f}" for yr in years],
            textposition='outside',
            textfont=dict(size=10),
        ))
    fig_multi.update_layout(
        barmode='group',
        title="Cumulative Loss Projection by Year",
        xaxis_title="Horizon",
        yaxis_title=f"Cumulative Loss ({currency})",
        height=450,
        legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1),
    ...
    for yr in years:
        d = multi_year[yr]
        multi_rows.append({
            'Horizon': f"{yr} year{'s' if yr > 1 else ''}",
            f'Mean ({currency})':    f"{currency}{d['mean']:,.0f}",
            f'50th pct ({currency})': f"{currency}{d['p50']:,.0f}",
            f'90th pct ({currency})': f"{currency}{d['p90']:,.0f}",
            f'95th pct ({currency})': f"{currency}{d['p95']:,.0f}",
            f'99th pct ({currency})': f"{currency}{d['p99']:,.0f}",
        })
    st.dataframe(pd.DataFrame(multi_rows), hide_index=True, use_container_width=True)
    st.caption(
        "💡 The 95th percentile column answers: "
        "'What total loss amount are we 95% confident we won't exceed over N years?' "
        "Use this to size multi-year cyber insurance programmes."
    )
    ...
    with col_save2:
        ...
        if st.button(
            ...
            st.session_state.saved_scenarios.append({
                ...
                'params': {
                    'cf': f"{cf_min:,}–{cf_mode:,}–{cf_max:,}/yr",
                    'poa': f"{poa*100:.1f}%",
                    'tef (derived)': f"{tef_min:,.1f}–{tef_mode:,.1f}–{tef_max:,.1f}",
    ...
    with col1:
        export_data = {
            ...
            "simulation_parameters": {
                "n_simulations": n_simulations,
                "random_seed": random_seed,
                "contact_frequency": {"min": cf_min, "mode": cf_mode, "max": cf_max},
                "probability_of_action": poa,
                "tef_derived": {"min": tef_min, "mode": tef_mode, "max": tef_max},
    ...
    with col3:
        html_report = generate_html_report(
            ...
            n_simulations=n_simulations,
            random_seed=random_seed,
            cf_min=cf_min, cf_mode=cf_mode, cf_max=cf_max, poa=poa,
    ...
    with col4:
        # Save current parameter configuration as JSON (for reloading later)
        config_data = {
            "version": "1.0",
            "schema_version": 2,  # CF + PoA model — see CHANGELOG.md
            "scenario_name": scenario_preset if scenario_preset != "Custom" else save_label,
            "parameters": {
                "cf_min": cf_min,
                "cf_mode": cf_mode,
                "cf_max": cf_max,
                "poa_pct": round(poa * 100, 2),
                "vulnerability_pct": round(total_vulnerability * 100, 4),
                ...
                "secondary_prob_pct": round(secondary_prob * 100, 2),
            }
        }
        st.download_button(
            label="⚙️ Save Config",
            data=json.dumps(config_data, indent=2),
            file_name=f"{client_name.replace(' ', '_')}_config.json",
            mime="application/json",
            help="Save current parameter configuration to reload later via the sidebar."
        )
...
if len(st.session_state.saved_scenarios) >= 2:
    ...
    with st.expander("🔧 Scenario Parameters"):
        # Column labels are taken from each scenario's own params dict keys
        # (cf, poa, tef (derived), vuln, primary, secondary_prob) rather than
        # a hardcoded index list, since a mismatch there would silently
        # mislabel columns.
        params_df = pd.DataFrame({s['name']: s['params'] for s in saved}).T
...
st.markdown("""
    <div style='text-align: center; color: #666; padding: 2rem 0;'>
        ...
        <p>Created for BARE Cybersecurity | Version 1.3</p>
